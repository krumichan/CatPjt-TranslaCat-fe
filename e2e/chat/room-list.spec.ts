...
test.describe("Chat room list", () => {
    ...
    test("ROOM-01~04 MANUAL/FRIEND DIRECT/GROUP를 현재 정책으로 표시한다", async ({
        ...
        await expect(
            page.getByText("Manual Direct"),
        ).toBeVisible();
        await expect(
            page.getByText("Manual Group"),
        ).toBeVisible();
        await expect(
            page.getByText("친구와의 1:1 채팅"),
        ).toBeVisible();
        await expect(
            page.getByText("친구 그룹"),
        ).toBeVisible();
        await expect(
            page.getByText("Phase 1.5 group"),
        ).toBeVisible();
    });

    test("ROOM-05 방 카드를 클릭하면 상세로 이동한다", async ({
        page,
    }) => {
    ...
    test("BASE-05 목록 조회 실패 후 재시도한다", async ({
        page,
    }) => {
        let count = 0;
