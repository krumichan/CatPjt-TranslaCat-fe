...
const mockAuthSecret = nextAuthSecret;

export async function installMockAuthCookie(
    ...
    const token = await encode({
        ...
        token: {
            ...
            iat: now,
            exp: now + maxAge,
            jti: `translacat-e2e-${user.key}-${role}-${now}`,
        },
    });
...
export const test = base.extend<MockAuthFixtures>({
    mockAuthCookie: [
        async ({ context }, use) => {
            await installMockAuthCookie(context);
