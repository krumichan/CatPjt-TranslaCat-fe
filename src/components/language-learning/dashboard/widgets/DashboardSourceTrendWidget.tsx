            ...
            <DisclosureContent id={contentId} isOpen={disclosure.isOpen}>
                <div className="mt-4 flex flex-wrap gap-2">
                    <AppSelect
                        ...
                        onChange={(event) =>
                            onSourceChange(event.target.value as DashboardSourceFilter)
                        }
                    ...
                    >
                        ...
                        <option value="VOCABULARY">{t("source.VOCABULARY")}</option>
                    </AppSelect>
                    <AppSelect
                        value={period}
                ...
                {data.collectingData || entries.length === 0 ? (
                    <p className="mt-6 text-sm text-slate-400">{t("collectingData")}</p>
                ) : (
                    <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                        {entries.map(([metric, points]) => {
                            const latest = points.at(-1)?.score ?? null;
                            return (
                                <div key={metric} className="rounded-2xl bg-slate-50 p-4 dark:bg-white/5">
                                    <p className="text-xs font-black uppercase text-slate-400">{metricT(metric)}</p>
                                    <p className="mt-2 text-2xl font-black text-slate-800 dark:text-slate-100">
                                        {latest === null ? "—" : Math.round(latest)}
                                    </p>
                                    <p className="mt-1 text-[11px] text-slate-400">{t("points", { count: points.length })}</p>
                                </div>
                            );
                        })}
                    </div>
