    ...
    return (
        <section className="rounded-3xl border border-slate-200 bg-white/90 p-6 shadow-sm dark:border-white/10 dark:bg-slate-900/75">
            ...
            <div className="mt-6 grid gap-5 md:grid-cols-2">
                ...
                <label className="block">
                    <span className="text-sm font-black text-slate-700 dark:text-slate-200">{t("dailySentenceCount")}</span>
                    <input
                        type="number"
                        ...
                        onChange={(event) => controller.update("dailySentenceCount", Number(event.target.value))}
                    ...
                    />
                    <span className="mt-1 block text-xs text-slate-400">{t("dailySentenceRange", { min: setting.minDailySentenceCount, max: setting.maxDailySentenceCount })}</span>
                </label>

                <label className="block">
                    <span className="text-sm font-black text-slate-700 dark:text-slate-200">{t("timezone")}</span>
                    <input
                        type="text"
                        value={form.timezone}
                        onChange={(event) => controller.update("timezone", event.target.value)}
                        className="mt-2 w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-900 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200 dark:border-white/10 dark:bg-black/20 dark:text-white dark:focus:ring-blue-500/20"
                    />
                </label>
            </div>
            ...
            {form.originLanguage === form.learningLanguage && (
                <p role="alert" className="mt-4 text-sm font-bold text-rose-600 dark:text-rose-300">{t("sameLanguage")}</p>
            )}
            {controller.saveError && (
                <p role="alert" className="mt-4 text-sm font-bold text-rose-600 dark:text-rose-300">{t("saveFailed")}</p>
            )}
            {controller.saved && (
                <p role="status" className="mt-4 text-sm font-bold text-emerald-600 dark:text-emerald-300">
                    {setting.configured ? t("savedNextDay") : t("savedInitial")}
                </p>
            )}
