...
export function SpeakingEvaluationView({
    ...
    if (session?.session.resultKind === "SESSION_COACHING") {
        const resultStatus = session.session.resultStatus;
        if (resultStatus === "PENDING" || resultStatus === "RUNNING") {
            return (
                <section data-testid="speaking-coaching-pending" className="rounded-3xl border border-blue-200 bg-white p-10 text-center dark:border-blue-400/20 dark:bg-slate-900">
                    <LoaderCircle className="mx-auto h-10 w-10 animate-spin text-blue-600" aria-hidden="true" />
                    <h2 className="mt-4 text-lg font-black text-slate-900 dark:text-white">{t("coaching.pendingTitle")}</h2>
                    <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">{t("coaching.pendingDescription")}</p>
                </section>
            );
        }
        if (resultStatus === "FAILED") {
            return (
                <section className="rounded-3xl border border-rose-200 bg-white p-8 text-center dark:border-rose-400/20 dark:bg-slate-900">
                    ...
                    <h2 className="mt-4 text-lg font-black text-slate-900 dark:text-white">{t("coaching.failedTitle")}</h2>
                    <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">{t("coaching.failedDescription")}</p>
                    <button type="button" disabled={controller.isRetrying} onClick={() => void controller.retry()} className="mt-5 inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-black text-white disabled:opacity-50">
                        ...
                        {controller.isRetrying ? t("retrying") : t("coaching.retry")}
                    </button>
                </section>
            );
        }
        if (session.coachingResult) return <SpeakingCoachingResultView result={session.coachingResult} />;
    }

    if (status === "NOT_REQUESTED") {
        return (
            <section data-testid="speaking-evaluation-not-requested" className="rounded-3xl border border-slate-200 bg-white p-8 text-center dark:border-white/10 dark:bg-slate-900">
                <h2 className="text-lg font-black text-slate-900 dark:text-white">{t("notRequestedTitle")}</h2>
                <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">{t("notRequestedDescription")}</p>
            </section>
        );
    }
    ...
    if (status === "FAILED") {
        return (
            <section className="rounded-3xl border border-rose-200 bg-white p-8 text-center dark:border-rose-400/20 dark:bg-slate-900">
                ...
                <button type="button" data-testid="speaking-evaluation-retry" disabled={controller.isRetrying} onClick={() => void controller.retry()} className="mt-5 inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-black text-white disabled:opacity-50">
                    <RotateCcw className="h-4 w-4" aria-hidden="true" />
    ...
    if (status === "INSUFFICIENT_EVIDENCE") {
        return (
            <section data-testid="speaking-evaluation-insufficient" className="rounded-3xl border border-amber-200 bg-white p-8 text-center dark:border-amber-400/20 dark:bg-slate-900">
                ...
                {evaluation && <div className="mt-4 text-left"><SpeakingEvidenceNotice evidence={evaluation} readAloud={controller.session?.session.practiceMode === "READ_ALOUD"} /></div>}
            </section>
        );
    }
